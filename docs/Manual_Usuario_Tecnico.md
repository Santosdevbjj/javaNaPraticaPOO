...
Credenciais de MySQL (root/admin) e RabbitMQ (guest/guest) são definidas APENAS no docker-compose.yml (variáveis de ambiente), e o código Java as lê via application.properties (que aponta para as variáveis de ambiente do container).
